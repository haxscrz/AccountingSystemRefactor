...
function buildJournal() {
  ...
  // Modify handleQuit to allow force exit
  content = content.replace(/const handleQuit = \(\) => \{\s*if \(!isBalanced\) \{\s*alert\([^;]+\);\s*return\s*\}\s*window\.history\.back\(\)\s*\}/, `const handleQuit = () => {
    if (!isBalanced) {
  ...
  // Upgrade message system
  content = content.replace(/const \[message, setMessage\] = useState\(''\)/, `const [message, setMessage] = useState({ text: '', type: 'info', field: '' })
  ...
  // Replace all setMessage with showMsg
  content = content.replace(/setMessage\((`[^`]*`|'[^']*'|"[^"]*"|err.*|found.*|.*Error.*)\)/g, (m, p1) => `showMsg(${p1})`);

  // Re-write the Return Block
  const oldReturnRender = /return \(\s*<div className="card">[\s\S]*?<\/div>\s*\)\s*\}/;
          ...
          <table className="w-full text-left border-collapse" style={{ minWidth: '800px' }}>
            ...
            <tbody className="divide-y divide-outline-variant/10">
              {records.map((r, idx) => {
                ...
                return (
                  <tr key={r.id} onDoubleClick={() => startEdit(r)} className="hover:bg-primary/5 even:bg-surface-container-lowest/50 group transition-colors cursor-pointer border-b border-outline-variant/20">
  ...
  content = content.replace(oldReturnRender, newReturnBlock);
  fs.writeFileSync('src/components/fs/FSJournalEntry.tsx', content);
}

function buildVoucher() {
  // We will fully rewrite the Voucher render block natively with tailwind and qol classes here:
  let content = fs.readFileSync('src/components/fs/FSVoucherEntry.tsx', 'utf8');
  
  // Actually, wait, Voucher Entry is HUGE. Instead of a full return block, let me use multi_replace for specific parts of Voucher entry later.
  // But wait, the `qol_fixes.mjs` DID run earlier and it tried to modify Voucher! AND IT FAILED WITH SYNTAX ERRORS in Voucher Entry.
  // So VOUCHER ENTRY WAS RETAINED by Git Checkout because I just `git checkout` it too!
  // I must rewrite the message state for it.
  ...
  content = content.replace(/setMessage\((`[^`]*`|'[^']*'|"[^"]*"|err.*|.*Error.*)\)/g, (m, p1) => `showMsg(${p1})`);

  // Allow forceful exit logic
  content = content.replace(/const handleQuit = \(\) => \{\s*if \(!isLineBalanced\) \{\s*setShowUnbalanced\(true\);\s*return;\s*\}\s*window\.history\.back\(\)\s*\}/, `const handleQuit = () => {
    if (!isLineBalanced) {
  ...
  // The 'Unbalanced' modal is where we let them exit anyway
  // Update Unbalanced modal buttons:
  const oldModalBtns = /<div className="bg-surface px-6 py-4 border-t border-outline-variant\/20 flex justify-end gap-3">\s*<button onClick=\{\(\) => setShowUnbalanced\(false\)\} className="px-5 py-2\.5 bg-surface-container hover:bg-surface-container-high text-on-surface font-bold text-sm rounded-lg transition-colors">\s*Return to Fix\s*<\/button>\s*<\/div>/g;
  ...
  content = content.replace(oldModalBtns, newModalBtns);

  // Replace {message} with the Toast
  const oldToastMsg = /\{message && \(\s*<div[^>]+>\s*<span[^>]+>\s*\{message\.includes\('Error'\)[^}]+\}\s*<\/span>\s*\{message\}\s*<\/div>\s*\)\}/;
  ...
  content = content.replace(oldToastMsg, newToastMsg);

  // Add IDs for highlights
  ...
  content = content.replace(/value=\{addLineRow.acctCode\}/g, `id="input_lineAcct" value={addLineRow.acctCode} className={\`\${message.field === 'lineAcct' ? 'ring-2 ring-error !border-error animate-pulse' : ''} \` + `);
  ...
  content = content.replace(/value=\{addLineRow.credit\}/g, `id="input_lineCredit" value={addLineRow.credit} className={\`\${message.field === 'lineCredit' ? 'ring-2 ring-error !border-error' : ''} \` + `);

  // Replace manual strings validating
  ...
  // Fix Variance display in modal: it was mapping {Math.abs(chk.balance).toLocaleString...} but it shouldn't be 0
  // In unbalancedchecks: unb.push({ ckNo: m.vCkNo, balance: db - cr })
  // What if it is 0.00? In VoucherEntry check the loop calculating db and cr.
...
console.log("Journal fully rewritten; Voucher updated.");
