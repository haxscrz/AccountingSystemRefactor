...
public sealed class FSController : ControllerBase
{
    ...
    public async Task<IActionResult> SeedFromLegacy(CancellationToken cancellationToken)
    {
        ...
        // ── helpers ──────────────────────────────────────────────────────────
        string Str(Dictionary<string, object?> row, string key, string def = "")
        {
            ...
            {
                JsonElement je => je.ValueKind == JsonValueKind.String ? (je.GetString() ?? def).Trim() : je.ToString().Trim(),
                string s => s.Trim(),
                _ => (kv.Value?.ToString() ?? def).Trim()
            };
        }

        decimal Dec(Dictionary<string, object?> row, string key)
        {
            var kv = row.FirstOrDefault(x => x.Key.Equals(key, StringComparison.OrdinalIgnoreCase));
        ...
        DateTime Dt(Dictionary<string, object?> row, string key)
        {
            var kv = row.FirstOrDefault(x => x.Key.Equals(key, StringComparison.OrdinalIgnoreCase));
            if (kv.Key is null) return now;
            var s = kv.Value switch
            {
                JsonElement je => je.ValueKind == JsonValueKind.String ? je.GetString() : je.ToString(),
                _ => kv.Value?.ToString()
            };
            return DateTime.TryParse(s, out var dt) ? dt : now;
        }

        bool Bool(Dictionary<string, object?> row, string key)
        {
            var kv = row.FirstOrDefault(x => x.Key.Equals(key, StringComparison.OrdinalIgnoreCase));
            ...
            return kv.Value switch
            {
                ...
                JsonElement je when je.ValueKind == JsonValueKind.String => je.GetString() == "T" || je.GetString() == "true" || je.GetString() == "1",
                bool b => b,
                _ => false
            };
        }
        ...
        // ── fs_accounts ──────────────────────────────────────────────────────
        var acDs = await _legacyDataService.GetDatasetByKeyAsync("fs_accounts", cancellationToken);
        if (acDs != null)
        {
        ...
        // ── fs_cashrcpt ──────────────────────────────────────────────────────
        var crDs = await _legacyDataService.GetDatasetByKeyAsync("fs_cashrcpt", cancellationToken);
        if (crDs != null)
        {
        ...
        // ── fs_salebook ──────────────────────────────────────────────────────
        var sbDs = await _legacyDataService.GetDatasetByKeyAsync("fs_salebook", cancellationToken);
        if (sbDs != null)
        {
        ...
        // ── fs_purcbook ──────────────────────────────────────────────────────
        var pbDs = await _legacyDataService.GetDatasetByKeyAsync("fs_purcbook", cancellationToken);
        if (pbDs != null)
        {
        ...
        // ── fs_adjstmnt ──────────────────────────────────────────────────────
        var adjDs = await _legacyDataService.GetDatasetByKeyAsync("fs_adjstmnt", cancellationToken);
        if (adjDs != null)
        {
        ...
        // ── fs_journals ──────────────────────────────────────────────────────
        var jnDs = await _legacyDataService.GetDatasetByKeyAsync("fs_journals", cancellationToken);
        if (jnDs != null)
        {
        ...
        var pjDs = await _legacyDataService.GetDatasetByKeyAsync("fs_pournals", cancellationToken);
        if (pjDs != null)
        {
        ...
        // ── fs_checkmas ──────────────────────────────────────────────────────
        var cmDs = await _legacyDataService.GetDatasetByKeyAsync("fs_checkmas", cancellationToken)
                ?? await _legacyDataService.GetDatasetByKeyAsync("fs_acheckma", cancellationToken);
        if (cmDs != null)
        {
        ...
        // ── fs_checkvou ──────────────────────────────────────────────────────
        var cvDs = await _legacyDataService.GetDatasetByKeyAsync("fs_checkvou", cancellationToken)
                ?? await _legacyDataService.GetDatasetByKeyAsync("fs_acheckvo", cancellationToken);
        if (cvDs != null)
        {
        ...
        // ── fs_effects ───────────────────────────────────────────────────────
        var efDs = await _legacyDataService.GetDatasetByKeyAsync("fs_effects", cancellationToken);
        if (efDs != null)
        {
        ...
        // ── fs_schedule ──────────────────────────────────────────────────────
        var scDs = await _legacyDataService.GetDatasetByKeyAsync("fs_schedule", cancellationToken);
        if (scDs != null)
        {
        ...
        var totalSeeded = seeded.Values.Sum();
        if (totalSeeded == 0)
        ...
        return Ok(new { message = $"Successfully seeded {totalSeeded} records across {seeded.Count} tables.", seeded });
    }
