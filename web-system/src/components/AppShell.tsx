          ...
          <div className="flex items-center gap-5">
            ...
            <div className="relative" ref={notifRef}>
              ...
              >
                ...
                {count > 0 && (
                  <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] bg-error rounded-full border-2 border-surface flex items-center justify-center text-[9px] font-bold text-white">
                    {count > 9 ? '9+' : count}
                  </span>
                )}
              </button>
              ...
              {showNotifPanel && (
                <div className={`absolute right-0 top-[calc(100%+8px)] w-[380px] rounded-xl shadow-2xl border z-50 overflow-hidden ${darkMode ? 'bg-[#1e2a4a] border-gray-600' : 'bg-white border-outline-variant/20'}`}>
                  ...
                  <div className="max-h-[360px] overflow-y-auto custom-scrollbar">
                    {notifications.length === 0 ? (
                      ...
                      notifications.map((n) => (
                        ...
                        >
                          <span className={`material-symbols-outlined text-[20px] mt-0.5 shrink-0 ${notifColor(n.type)}`}>
                            {notifIcon(n.type)}
                          </span>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-2">
                              <p className={`text-[13px] font-semibold leading-tight ${darkMode ? 'text-gray-100' : 'text-on-surface'}`}>{n.title}</p>
                              <div className="flex items-center gap-1 shrink-0">
                                <span className={`text-[10px] ${darkMode ? 'text-gray-500' : 'text-on-surface-variant/50'}`}>{formatTimeAgo(n.timestamp)}</span>
                                <button 
                                  onClick={(e) => { e.stopPropagation(); removeNotification(n.id) }}
                                  className="opacity-0 group-hover:opacity-100 transition-opacity"
                                >
                                  <span className={`material-symbols-outlined text-[14px] ${darkMode ? 'text-gray-500 hover:text-red-400' : 'text-on-surface-variant/40 hover:text-error'}`}>close</span>
                                </button>
                              </div>
                            </div>
                            <p className={`text-xs mt-0.5 leading-relaxed ${darkMode ? 'text-gray-400' : 'text-on-surface-variant/70'}`}>{n.message}</p>
                            {n.from && <p className={`text-[10px] mt-1 font-semibold ${darkMode ? 'text-gray-500' : 'text-on-surface-variant/40'}`}>From: {n.from}</p>}
                          </div>
